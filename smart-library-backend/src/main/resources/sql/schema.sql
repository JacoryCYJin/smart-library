...
-- 版本: V3.2 (Resource Aggregation & AI Graph)
...
DROP TABLE IF EXISTS resource_link; -- 资源-聚合链接
DROP TABLE IF EXISTS resource_character_graph; -- AI人物图谱
...
(
    ...
    has_graph       TINYINT       DEFAULT 0 COMMENT 'AI图谱标识: 0-无图谱 / 1-已生成图谱',
    ...
    INDEX idx_has_graph (has_graph)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='核心资源总表';

-- ==========================================
-- 2.1 资源聚合链接表 (Resource Link)
-- 说明: 作为"导航枢纽"，聚合外部资源链接
-- ==========================================
...
(
    ...
    link_type   TINYINT       NOT NULL COMMENT '链接类型: 1-书籍页 / 2-下载页 / 3-解读页',
    platform    TINYINT       NOT NULL COMMENT '平台: 1-豆瓣 / 2-ZLibrary / 3-B站 / 4-YouTube / 99-其他',
