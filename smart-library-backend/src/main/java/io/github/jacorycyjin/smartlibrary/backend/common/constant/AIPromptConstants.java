...
public class AIPromptConstants {
    ...
    public static final String CHARACTER_GRAPH_SYSTEM_PROMPT = """
            ...
            业务规则：
            1. 体裁判定前置：首先评估该书是否包含具体的人物实体和人物关系。
               - ✅ 适合生成图谱的类型：小说（虚构叙事）、历史著作（如《安史之乱》《明朝那些事儿》）、人物传记、历史演义、纪实文学、戏剧剧本等。
               - ❌ 不适合生成图谱的类型：纯理论学术专著（无具体人物）、技术手册、工具书、字典、纯哲学思辨、纯科学理论等。
               - 判定原则：只要书中涉及具体的历史人物、虚构角色或真实人物的互动关系，就应该尝试生成图谱。
            2. 核心人物过滤：根据书籍复杂度自主判断节点数量。仅提取推动核心剧情或历史事件发展的关键人物，坚决剔除边缘人物、出场极少的配角或背景板角色。
            3. 结构约束：仅输出包含 nodes 和 edges 的 JSON 对象，禁止任何 Markdown 标记或解释性文字。
            4. nodes 节点字段：id(拼音唯一标识)、label(人物名称)、category(所属阵营/派系，必须是单一明确的词汇，严禁包含 "/" 等符号，如必须把天庭/佛界提炼为神仙)、weight(1至100的整数，代表重要程度)。
            ...
            6. 兜底规则：仅在以下情况返回 {"nodes": [], "edges": []}：
               - 书籍确实不包含任何具体人物实体（如纯理论著作）
               - 知识库完全未收录该书且无法推断人物关系
    ...
    public static final String EMOTION_ARC_SYSTEM_PROMPT = """
            ...
            业务规则：
            1. 体裁判定前置：首先评估该书是否适合进行情感走向分析。
               - ✅ 适合分析的类型：小说（虚构叙事）、历史著作（如《安史之乱》《明朝那些事儿》）、人物传记、历史演义、纪实文学、散文集、戏剧剧本等。
               - ❌ 不适合分析的类型：纯理论学术论文（无情节）、技术手册、工具书、字典、纯哲学思辨、纯科学理论等。
               - 判定原则：只要书中包含事件发展、情节推进或情感变化（无论虚构或真实），就应该尝试分析情感走向。
            2. 章节粒度控制：根据书籍长度智能调整分析粒度。短篇（<10章）按章节分析；中篇（10-30章）每2-3章合并；长篇（>30章）每5-10章合并。根据书籍复杂度自主判断最终输出的章节数量。
            3. 情感评分标准：emotionScore 范围为 -1.0 到 +1.0，其中 -1.0 表示极度悲伤/绝望/低谷，0.0 表示平静/中性，+1.0 表示极度欢乐/激动/高潮。对于历史作品，可以根据历史事件的正面/负面影响来评分。
            ...
            9. 兜底规则：仅在以下情况返回 {"chapters": [], "overallTrend": "无法分析", "emotionalPeaks": [], "emotionalValleys": []}：
               - 书籍确实不包含任何情节或情感变化（如纯理论著作）
               - 知识库完全未收录该书且无法推断情感走向
